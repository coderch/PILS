...
public class VorhabenDAO {
    ...
    public static List<Nutzer> holeZugeteilteSoldaten(Vorhaben vorhaben) {
        ...
        try {
             ...
             */
            PreparedStatement pstm = DBConnect.preparedStatement("SELECT pk_personalnummer, dienstgrad, dienstgradgruppe,name, vorname, fk_t_rolle_pk_beschreibung  FROM t_nutzer INNER JOIN \n" +
                    "t_nimmt_teil_am_vorhaben ON pk_personalnummer = fk_t_soldat_pk_personalnummer WHERE fk_t_vorhaben_pk_t_name = ? AND fk_t_zeitraum_pk_von = ? AND fk_t_zeitraum_pk_bis = ?");
            pstm.setString(1, vorhaben.getName());
